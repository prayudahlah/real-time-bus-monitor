...
def main(**kwargs):
    ...
    logger.info("Reading agency.parquet from MinIO...")
    response = client.get_object("raw-data", f"{run_id}/agency.parquet")
    ...
    with conn.cursor() as cur:
        logger.info("Truncating table agency...")
        cur.execute("TRUNCATE TABLE agency;")

        logger.info("Copying to PostgreSQL...")
        reader = CopyProgressReader(csv_str, len(csv_str), "agency")
        cur.copy_expert(
